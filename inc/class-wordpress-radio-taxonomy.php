...
if (!class_exists('WordPress_Radio_Taxonomy')) :
	...
	class WordPress_Radio_Taxonomy {
		...
		/**
		 * @var string - post taxonomy array
		 * @since 2.5.0
		 */
		...
		public function __construct($taxonomy) {
			...
			$opt = Radio_Buttons_For_Taxonomies()->get_options();
			if (isset($opt['taxonomies'][$taxonomy])) {
				$this->post_taxonomy = $opt['taxonomies'][$taxonomy];
			} else {
				$this->post_taxonomy = get_taxonomy($taxonomy)->object_type;
			}
...
<?php
		...
		public function add_non_hierarchical_term() {
			...
			foreach ($names as $cat_name) {
				...
				$data = sprintf(
					'<li id="%1$s-%2$s"><label class="selectit"><input id="in-%1$s-%2$s" type="radio" name="radio_tax_input[%1$s][]" value="%2$s" checked="checked"> %3$s</label></li>',
