...
class Radio_Buttons_For_Taxonomies {
	...
	public function block_editor_assets() {
		...
		$i18n = array('radio_taxonomies' => $localize_taxonomies);
	...
	/**
	 * Checks if there are values in nested arrays
	 * @access public
	 * @return bool
	 * @since  2.5.0
	 */
	...
	public function get_options($option = false) {
		if (!$this->options) {
			...
			foreach ($this->options['taxonomies'] as $key => $taxonomy) {
				if (is_string($taxonomy) && !isset($this->options['taxonomies'][$taxonomy])) {
					$this->options['taxonomies'][$taxonomy] = get_taxonomy($taxonomy)->object_type;
				}
			}
		}
